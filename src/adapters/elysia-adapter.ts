...
const formDataToObject = (formData: globalThis.FormData): Record<string, unknown> => {
...
> {
  ...
  private serverReadyCallbacks = new Set<(server: HttpServer | undefined) => void>();
  ...
  public getNodeServer(): HttpServer | undefined {
    const serverInfo = this.instance.server as ElysiaListeningServer | null | undefined;
  ...
  private registerLayer(method: Method, path: string, handler: LayerHandler) {
    ...
    this.getOnRouteTriggered()?.(
      method === 'USE' ? RequestMethod.ALL : (RequestMethod[method as keyof typeof RequestMethod] ?? RequestMethod.ALL),
      normalizedPath,
    );
  }
  ...
  private createRequest(
    ...
    return {
      ...
      formData: () => request.clone().formData() as Promise<globalThis.FormData>,
  ...
  private getParserConfig(contentType: string | null): ParserConfig | undefined {
    if (!contentType) {
      return undefined;
    }
  ...
  private async dispatch(request: Request): Promise<Response> {
    ...
    const matchingLayers = this.layers
      ...
      .filter(Boolean) as Array<{ layer: Layer; params: Record<string, string> }>;
  ...
  public override use(
    ...
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('USE', path, routeHandler);
    return this;
  }

  public override get(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('GET', path, routeHandler);
    return this;
  }

  public override post(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('POST', path, routeHandler);
    return this;
  }

  public override head(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('HEAD', path, routeHandler);
    return this;
  }

  public override put(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('PUT', path, routeHandler);
    return this;
  }

  public override delete(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('DELETE', path, routeHandler);
    return this;
  }

  public override patch(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('PATCH', path, routeHandler);
    return this;
  }

  public override options(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('OPTIONS', path, routeHandler);
    return this;
  }

  public override all(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('ALL', path, routeHandler);
    return this;
  }

  public override search(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('SEARCH', path, routeHandler);
    return this;
  }

  public override propfind(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('PROPFIND', path, routeHandler);
    return this;
  }

  public override proppatch(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('PROPPATCH', path, routeHandler);
    return this;
  }

  public override mkcol(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('MKCOL', path, routeHandler);
    return this;
  }

  public override copy(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('COPY', path, routeHandler);
    return this;
  }

  public override move(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('MOVE', path, routeHandler);
    return this;
  }

  public override lock(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('LOCK', path, routeHandler);
    return this;
  }

  public override unlock(pathOrHandler: string | LayerHandler, handler?: LayerHandler) {
    const [path, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);
    this.registerLayer('UNLOCK', path, routeHandler);
    return this;
  }

  public async reply(response: ElysiaReply, body: unknown, statusCode?: number) {
  ...
  public setNotFoundHandler(handler: RequestHandler<ElysiaRequest, ElysiaReply>) {
  ...
  public getRequestUrl(request: ElysiaRequest): string {
    return request.originalUrl ?? `${new URL(request.url).pathname}${new URL(request.url).search}`;
  }
