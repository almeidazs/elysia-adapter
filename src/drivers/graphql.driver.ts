...
import type { ElysiaReply, ElysiaRequest } from '../types';
import {
...
export class ElysiaGraphQLDriver<
	// biome-ignore lint/suspicious/noExplicitAny: Nest's AbstractGraphQLDriver requires this broad config constraint.
...
> extends AbstractGraphQLDriver {
	...
	async start(options: T): Promise<void> {
		...
		if (httpAdapter.getType() !== 'elysia') {
			throw new Error(
				'This driver is only compatible with the Elysia platform',
			);
		}
	...
	private getSchema(configuredSchema?: GraphQLSchema): GraphQLSchema {
		...
		const schema =
			internals.internals?.state?.schemaManager?.getSchemaDerivedData?.()
				.schema;
	...
	private async parseBody(
		...
		if (contentType?.startsWith('application/json')) {
			...
			return req.json().catch(this.logError) as Promise<
